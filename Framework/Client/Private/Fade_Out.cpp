...
CFade_Out::CFade_Out(const CFade_Out & rhs)
	: CGameObject(rhs)
{
}
...
HRESULT CFade_Out::Initialize(void * pArg)
{
...
void CFade_Out::Tick(_float fTimeDelta)
{
...
HRESULT CFade_Out::SetUp_Components()
{
	/* For.Com_Shader */
...
HRESULT CFade_Out::SetUp_ShaderResource()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
CFade_Out * CFade_Out::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	...
	{
		MSG_BOX("Failed to Created : CFade_In");
...
CGameObject * CFade_Out::Clone(void * pArg)
{
	...
	{
		MSG_BOX("Failed to Cloned : CFade_Out");
		Safe_Release(pInstance);
	}
