...
void CMonster_Bullet::Tick(_float fTimeDelta)
{
...
void CMonster_Bullet::LateTick(_float fTimeDelta)
{
	...
	for (_uint i = 0; i < pGameInstance->Get_GameObjectSize(LEVEL_GAMEPLAY, TEXT("Layer_Student")); ++i)
	{
		...
		CCollider* pCollider = (CCollider*)pStudent->Get_Component(TEXT("Com_AABB"));
...
HRESULT CMonster_Bullet::SetUp_Components()
{
	/* For.Com_Shader */
...
HRESULT CMonster_Bullet::SetUp_ShaderResource()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
HRESULT CMonster_Bullet::InitLook()
{
	...
	if (pStudent)
	{
		vTarget = pStudent->Get_StudentTranslation();
...
void CMonster_Bullet::OnDisable()
{
}

void CMonster_Bullet::OnEnable()
{
}
...
CGameObject * CMonster_Bullet::Clone(void * pArg)
{
	...
	{
		MSG_BOX("Failed to Cloned : CMonster_Bullet");
		Safe_Release(pInstance);
	}
