#include "stdafx.h"
#include "..\Public\Camera_Haruka.h"
...
CCamera_Haruka::CCamera_Haruka(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CCamera_Ex(pDevice, pContext)
{
}

CCamera_Haruka::CCamera_Haruka(const CCamera_Haruka & rhs)
	: CCamera_Ex(rhs)
{
}

HRESULT CCamera_Haruka::Initialize_Prototype()
{
...
HRESULT CCamera_Haruka::Initialize(void * pArg)
{

	if (nullptr != pArg)
		m_pHarukaEx = (CStudent*)pArg;

	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);

	CModel*		pTargetModel = (CModel*)m_pHarukaEx->Get_Component(TEXT("Com_Model"));

	m_pBonePtr = pTargetModel->Find_HierarcyNode("Camera001");
	m_pTargetBonePtr = pTargetModel->Find_HierarcyNode("Camera001.Target");

	m_pTargetTransform = (CTransform*)m_pHarukaEx->Get_Component(TEXT("Com_Transform"));
...
void CCamera_Haruka::Tick(_float fTimeDelta)
{
...
void CCamera_Haruka::LateTick(_float fTimeDelta)
{
	__super::LateTick(fTimeDelta);
}

HRESULT CCamera_Haruka::Render()
{
	return S_OK;
}

void CCamera_Haruka::OnDisable()
{

}

void CCamera_Haruka::OnEnable()
{

}

void CCamera_Haruka::MoveCamera(_float fTimeDelta)
{
...
CCamera_Haruka * CCamera_Haruka::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CCamera_Haruka*		pInstance = new CCamera_Haruka(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CCamera_Haruka");
...
CGameObject * CCamera_Haruka::Clone(void * pArg)
{
	CCamera_Haruka*		pInstance = new CCamera_Haruka(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CCamera_Haruka");
		Safe_Release(pInstance);
	}
