#include "stdafx.h"
#include "..\Public\Camera_Mutsuki.h"
...
CCamera_Mutsuki::CCamera_Mutsuki(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CCamera_Ex(pDevice, pContext)
{
}

CCamera_Mutsuki::CCamera_Mutsuki(const CCamera_Mutsuki & rhs)
	: CCamera_Ex(rhs)
{
}

HRESULT CCamera_Mutsuki::Initialize_Prototype()
{
...
HRESULT CCamera_Mutsuki::Initialize(void * pArg)
{

	if (nullptr != pArg)
		m_pMutsukiEx = (CStudent*)pArg;

	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);

	CModel*		pTargetModel = (CModel*)m_pMutsukiEx->Get_Component(TEXT("Com_Model"));

	m_pBonePtr = pTargetModel->Find_HierarcyNode("Camera001");
	m_pTargetBonePtr = pTargetModel->Find_HierarcyNode("Camera001.Target");

	m_pTargetTransform = (CTransform*)m_pMutsukiEx->Get_Component(TEXT("Com_Transform"));
...
void CCamera_Mutsuki::Tick(_float fTimeDelta)
{
...
void CCamera_Mutsuki::LateTick(_float fTimeDelta)
{
	__super::LateTick(fTimeDelta);
}

HRESULT CCamera_Mutsuki::Render()
{
	return S_OK;
}

void CCamera_Mutsuki::OnDisable()
{

}

void CCamera_Mutsuki::OnEnable()
{

}

void CCamera_Mutsuki::MoveCamera(_float fTimeDelta)
{
...
CCamera_Mutsuki * CCamera_Mutsuki::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CCamera_Mutsuki*		pInstance = new CCamera_Mutsuki(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CCamera_Mutsuki");
...
CGameObject * CCamera_Mutsuki::Clone(void * pArg)
{
	CCamera_Mutsuki*		pInstance = new CCamera_Mutsuki(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CCamera_Mutsuki");
		Safe_Release(pInstance);
	}
